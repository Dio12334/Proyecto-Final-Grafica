...
void Game::loadData(){
	...
	auto tc = new TunnelMeshComponent(e);
	std::vector<int> pointsPerCylinder;
	auto tunnel = GenerateTunnel(pointsPerCylinder);
	auto indices = GenerateIndices(tunnel, pointsPerCylinder);
	std::cout << tunnel.size() << std::endl;
	VertexArray* va = new VertexArray(tunnel.data(), tunnel.size(), indices.data(), indices.size());
	tc->setVertexArray(va);
	tc->setMesh(mesh);
	...
	/* a->setPosition(Vector3(200,200,200)); */
	/* mesh = _renderer->getMesh("Assets/Sphere.gpmesh"); */
	/* me = new MeshComponent(a); */
	/* me->setMesh(mesh); */
